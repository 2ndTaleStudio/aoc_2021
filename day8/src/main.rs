...
fn part2() {
    let result = include_str!("./input.txt")
        ...
        .map(|sample| {
            let one = *sample
                .iter()
                .find(|segments| segments.count_ones() == 2)
                .unwrap();
            let four = *sample
                .iter()
                .find(|segments| segments.count_ones() == 4)
                .unwrap();
            let seven = *sample
                .iter()
                .find(|segments| segments.count_ones() == 3)
                .unwrap();
            let eight = *sample
                .iter()
                .find(|segments| segments.count_ones() == 7)
                .unwrap();

            // get the 6 segment digits
            let six_segs = sample
                .iter()
                .filter(|segments| segments.count_ones() == 6)
                .collect::<Vec<_>>();
            // the one that overlaps with only 1 segments with the 1 is the 6
            let six = **six_segs
                .iter()
                .find(|&&&segments| (segments & one).count_ones() == 1)
                .unwrap();
            // the one that overlaps with all 4 segments with the 4 is the 9
            let nine = **six_segs
                .iter()
                .find(|&&&segments| (segments & four).count_ones() == 4)
                .unwrap();
            // the one that is neither 6 or 9 must be the zero
            let zero = **six_segs
                .iter()
                .find(|&&&segments| segments != nine && segments != six)
                .unwrap();

            // get the 5 segment digits
            let five_segs = sample
                .iter()
                .filter(|segments| segments.count_ones() == 5)
                .collect::<Vec<_>>();
            // the one that overlaps with 2 segments with the 1 is the 3
            let three = **five_segs
                .iter()
                .find(|&&segments| (segments & one).count_ones() == 2)
                .unwrap();
            // the one that overlaps all 5 segments with the 6 is the 5
            let five = **five_segs
                .iter()
                .find(|&&segments| (segments & six).count_ones() == 5)
                .unwrap();
            // the one that is neither three or five must be the two
            let two = **five_segs
                .iter()
                .find(|&&&segments| segments != three && segments != five)
                .unwrap();
        ...
        .zip(include_str!("./input.txt").lines().map(|line| {
            line.split(" | ")
                .nth(1)
                .unwrap()
                .split_ascii_whitespace()
...
fn segments_to_bits(segments: &str) -> i32 {
    segments.chars().fold(0_i32, |acc, part| match part {
        'a' => acc | 0b1,
        'b' => acc | (0b1 << 1),
        'c' => acc | (0b1 << 2),
        'd' => acc | (0b1 << 3),
