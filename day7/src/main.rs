...
fn part1() {
    let positions = include_str!("./input.txt").split(",").map(|v| v.parse::<i32>().unwrap()).collect::<Vec<_>>();
    let mut position_counts = positions.iter().fold(HashMap::new(), |mut acc, pos| {
        *acc.entry(pos).or_insert(0) += 1;
        acc
    }).into_iter().collect::<Vec<_>>();
    position_counts.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
    let fuels = position_counts.iter().map(|(&pos, _)| {
        positions.iter().fold(0, |acc, p| acc + i32::abs(p - pos))
    }).min();
...
fn part2() {
    ...
    let fuels = (min_pos..max_pos).map(|pos| {
