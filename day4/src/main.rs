...
fn apply_drawing(boards: &mut Vec<Vec<Option<i32>>>, draw: i32) {
    for board in boards.iter_mut() {
        if let Some(cell) = board.iter_mut()
            .find(|cell| 
                match cell {
...
fn part1() {
    ...
    let drawing = input.lines().nth(0).unwrap()
        .split(",")
    ...
    let mut boards = input.chunks(6)
            .map(|rows| {
                rows.iter()
                    .filter(|&&c| c != "")
                    .map(|cols| cols.split_ascii_whitespace()
    ...
    for draw in drawing {
        ...
        let wb = boards.iter().filter(|board| {
            let row_full = board.chunks_exact(5).filter(|v| v.iter().filter(|c| c.is_none()).count() == 5).count() != 0;
            let column_full = (0..5).into_iter().fold(false, |acc, col| {
                acc | (board.iter().skip(col).step_by(5).filter(|c| c.is_none()).count() == 5)
            });
    ...
    if let Some(wb) = winner_board {
        let points = wb.iter().fold(0, |acc, cell| {
            if let Some(v) = cell {
                acc + v
            } else {
                acc
...
fn part2() {
    ...
    for draw in drawing {
        ...
        let open_boards = boards.iter().filter(|board| {
            let row_full = board.chunks_exact(5).filter(|v| v.iter().filter(|c| c.is_none()).count() == 5).count() != 0;
            let column_full = (0..5).into_iter().fold(false, |acc, col| {
                acc | (board.iter().skip(col).step_by(5).filter(|c| c.is_none()).count() == 5)
            });
